...
def remove_empty_first_rows(worksheet: Worksheet) -> Worksheet:
...
class SchemaInfo(TypedDict):
    ...
    column: int
    overall: NotRequired[typing.Optional[typing.Union[int, float]]]


class SubjectSchema(TypedDict):
...
class BroadSheetSchema(TypedDict):
...
def _default_subject_schema() -> SubjectSchema:
    """Return a default subject schema with all scores initialized to zero."""
    return SubjectSchema(
        mid_term_score=SchemaInfo(column=0, overall=None),
        exam_score=SchemaInfo(column=0, overall=None),
        total_score=SchemaInfo(column=0, overall=None),
    )


def _default_schema_info() -> SchemaInfo:
    """Return a default schema info with column index set to 0 and overall value as None."""
    return SchemaInfo(column=0, overall=None)


def get_broadsheet_schema(worksheet: Worksheet) -> BroadSheetSchema:
    """
    ...
    schema = BroadSheetSchema(
        ...
        subjects=defaultdict(_default_subject_schema),
        aggregates=defaultdict(_default_schema_info),
        teachers_comment=SchemaInfo(column=0, overall=None),
        coordinators_comment=SchemaInfo(column=0, overall=None),
    )
    ...
    # Simply put, just the 2nd and 3rd row are what we need to extract the column schema.
    for cols in batched(
        ...
        for col in cols:
            ...
            sub_title_column_index = typing.cast(int, col[1].column)
            overall = col[2].value

            if title:
                title = _to_internal(str(title))
            else:
                if not previous_title:
                    if not sub_title:
                        continue

                    sub_title = _to_internal(str(sub_title))
                    ...
                    schema["aggregates"][sub_title]["overall"] = (
                        float(overall) if overall else None  # type: ignore
                    )
...
def students(worksheet: Worksheet) -> typing.Generator[StudentInfo, None, None]:
...
class SubjectScore(TypedDict):
    """Scores for each term section for a subject."""

    mid_term_score: typing.Optional[Score]
    exam_score: typing.Optional[Score]
    total_score: typing.Optional[Score]
    grade: typing.Optional[Grade]


SubjectsScores: TypeAlias = typing.Dict[str, SubjectScore]
"""Type alias for a collection or mapping of subject scores."""
AggregatesValues: TypeAlias = typing.Dict[str, typing.Optional[Score]]
...
def get_subjects_scores_for_student(
    ...
    for subject, subject_schema in subjects_schemas.items():
        ...
        subject_score = SubjectScore(
            mid_term_score=float(mid_term_score)  # type: ignore[arg-type]
            if mid_term_score is not None
            else None,
            exam_score=float(exam_score) if exam_score is not None else None,  # type: ignore[arg-type]
            total_score=float(total_score) if total_score is not None else None,  # type: ignore[arg-type]
            grade=get_grade(total_score) if total_score is not None else None,  # type: ignore[arg-type]
        )
...
def get_aggregates_values(
    ...
    for aggregate, aggregate_schema in aggregates_schemas.items():
        ...
        aggregates_values[aggregate] = (
            round(float(aggregate_value), ndigits=2)  # type: ignore[arg-type]
...
class BroadSheetData(TypedDict):
